      ...
      {isLoading ? (
        ...
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {columns.map((column, columnIndex) => { // Iterate over all static columns
              const { setNodeRef, isOver } = droppableRefs.current[column.id]; // Get refs from the pre-computed object

              // Filter tasks for the current column based on search/project filters
                ...
                >
                  <Card className={`${column.color} border-0 shadow-sm`}>
                    ...
                    <CardContent className="pt-0">
                      <SortableContext
                        items={columnTasks.map((task) => task.id)} // Only sortable items here
                        strategy={verticalListSortingStrategy}
                      >
                        <div
                          ref={setNodeRef} // Apply droppable ref to this div
                          className={`space-y-3 min-h-[400px] max-h-[600px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 ${
                            isOver ? 'border-2 border-blue-500 rounded-md' : '' // Visual feedback on hover
                          }`}
                        >
                          <AnimatePresence>
                            {columnTasks.map((task) => ( // Render filtered tasks
                              <TaskCard
                                key={task.id}
                                task={task}
                                ...
                                projects={projects} // Pass projects to TaskCard
                              />
                            ))}
                          </AnimatePresence>
                          {columnTasks.length === 0 && !searchQuery && (
                            // Placeholder for empty columns to allow dropping
                            <div
                              ...
                              style={{ minHeight: '100px' }} // Give it some height for visibility
                            >
                              Drag tasks here
                            </div>
                          )}
