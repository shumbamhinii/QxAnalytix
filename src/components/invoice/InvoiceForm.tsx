...
interface InvoiceLineItem {
  id?: string; // Optional for new line items
...
interface ProductService {
  ...
  name: string;
  description: string;
  price: number; // This is 'price' (number) from ProductFrontend, maps to unit_price (number)
  costPrice?: number;
  sku?: string;
  isService: boolean;
  stock: number;
...
interface InvoiceFormProps {
  invoice?: any; // Consider defining a more specific Invoice interface if available
  onClose: () => void;
...
export function InvoiceForm({ invoice, onClose, onSubmitSuccess }: InvoiceFormProps) {
  ...
  // Helper to calculate default due date (e.g., 7 days from now)
  const getDefaultDueDate = (invoiceDateString: string) => {
    const invoiceDate = new Date(invoiceDateString);
    const dueDate = new Date(invoiceDate);
    dueDate.setDate(invoiceDate.getDate() + 7); // Default 7 days later
  ...
  const [formData, setFormData] = useState<InvoiceFormData>({
    invoice_number: invoice ? invoice.invoice_number : generateInvoiceNumber(), // Autogenerate for new
    customer_id: null,
    customer_name_manual: '',
    invoice_date: initialInvoiceDate,
    due_date: getDefaultDueDate(initialInvoiceDate), // Set default due date
    status: 'Draft',
    currency: 'ZAR',
    notes: '',
    line_items: [],
  ...
  // Fetch Products/Services and Populate Form for Editing
  useEffect(() => {
    ...
    if (invoice) {
      setFormData({
        ...
        due_date: invoice.due_date ? new Date(invoice.due_date).toISOString().split('T')[0] : getDefaultDueDate(invoice.invoice_date || initialInvoiceDate), // Set default due date if not provided
        status: invoice.status || 'Draft',
        currency: invoice.currency || 'ZAR',
        notes: invoice.notes || '',
        line_items: invoice.line_items?.map((item: any) => ({
          ...item,
          // Ensure these are parsed as numbers and default to 0 if parsing fails
      ...
      } else if (invoice.customer_name) { // Use invoice.customer_name for manual if no customer_id
        setCustomerSearchQuery(invoice.customer_name);
      }
    }
  }, [invoice, toast]);

  // Fetch customer suggestions based on debounced search query
  useEffect(() => {
  ...
  const handleProductServiceSelect = (index: number, productId: string) => {
    ...
    if (product) {
      ...
      item.unit_price = product.price; // Use product.price, not product.unit_price
      item.quantity = item.quantity || 1;
      item.tax_rate = item.tax_rate || product.vatRate || 0.00; // Use product's vatRate if available
